...
function ReviewForm({
  ...
  onSaved: (review: ProductReviewRow) => void
}) {
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-1 text-[15px] font-semibold leading-snug text-[#111] sm:text-[16px]">
        {ownReview ? "Edita tu reseña" : "Comparte tu opinión"}
      </h3>
      ...
      <textarea
        ...
        className="mt-4 w-full resize-none rounded-xl border border-[#ececec] bg-white px-4 py-3 text-[14px] text-[#1a1a1a] outline-none placeholder:text-[#6b6b6b] transition-all focus:border-[#c6a75e] focus:shadow-[0_0_0_3px_#f5efdc]"
      />
...
export default function ProductReviews({
  ...
  // La página de producto se sirve cacheada, así que la elegibilidad (sesión +
  // compra verificada) y la frescura de la lista se resuelven en el cliente.
  const [canReview, setCanReview] = useState(false)
  const [own, setOwn] = useState<ProductReviewRow | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${productId}/reviews`)
      ...
      .then((json) => {
        ...
        setSummary(json.data.summary ?? { average: 0, count: 0 })
        setCanReview(Boolean(json.data.canReview))
        setOwn(json.data.ownReview ?? null)
      })
      .catch(() => {})
    return () => {
      cancelled = true
  ...
  function handleSaved(review: ProductReviewRow) {
    setOwn(review)
    setReviews((prev) => {
      const rest = prev.filter((r) => r.id !== review.id)
      const all = [review, ...rest]
  ...
  return (
    <section id="resenas" className="mt-16 scroll-mt-24">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[12.5px] text-[#6b6b6b]">
                {summary.count} reseña{summary.count !== 1 ? "s" : ""} con
                compra verificada
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_minmax(0,380px)]">
        ...
        {/* Formulario — solo clientas con compra verificada */}
        {canReview && (
          <div>
            <ReviewForm
              productId={productId}
