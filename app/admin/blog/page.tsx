...
type Panel = { mode: "create" } | { mode: "edit"; post: BlogPost }
...
export default function AdminBlogPage() {
  ...
  async function handleSave(data: BlogFormData) {
    ...
    try {
      ...
      const body = isEdit ? { id: (panel as { mode: "edit"; post: BlogPost }).post.id, ...data } : data
  ...
  async function handleToggleActive(post: BlogPost) {
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-10">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold text-[#1a1a1a]">Blog</h1>
            <p className="mt-1 text-sm text-[#6b6b6b]">
              Administra las publicaciones del blog: tips, tendencias, cuidado e inspiración.
            </p>
          </div>
        ...
        {!loading && !fetchError && posts.length === 0 && !panel && (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-200 py-20 text-center">
            <p className="text-[14px] text-neutral-400">Aún no hay publicaciones en el blog.</p>
            <button
              type="button"
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {posts.map((post) => {
              ...
              return (
                ...
                >
                  {/* Imagen */}
                  <div className="relative bg-neutral-100" style={{ aspectRatio: "3/2" }}>
                    {post.cover_image ? (
                      <Image
                  ...
                  <div className="flex flex-1 flex-col gap-2 p-4">
                    <span className={`w-fit rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.08em] ${category?.tag ?? "border-neutral-200 bg-neutral-100 text-neutral-500"}`}>
                      {post.category}
                    </span>
                    <h3 className="font-[family-name:var(--font-playfair),serif] text-[15px] font-medium leading-snug text-[#111]">
                      {post.title}
                    </h3>
                    {post.linked_products.length > 0 && (
                      <p className="text-[11px] text-neutral-400">
                        {post.linked_products.length} producto{post.linked_products.length !== 1 ? "s" : ""} relacionado{post.linked_products.length !== 1 ? "s" : ""}
                      </p>
                    )}
                  </div>
