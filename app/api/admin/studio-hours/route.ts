...
import {
  getStudioWeeklyHours,
  saveStudioWeeklyHours,
} from "@/lib/supabase/studio-hours"
import { studioWeeklyHoursUpdateSchema } from "@/lib/validations/appointments"
...
export async function GET() {
  try {
    ...
    const result = await getStudioWeeklyHours()
    if (!result.data) {
    ...
    console.error("[api/admin/studio-hours GET]", err)
...
export async function PUT(request: Request) {
  try {
    ...
    const parsed = studioWeeklyHoursUpdateSchema.safeParse(json)
    if (!parsed.success) {
    ...
    const result = await saveStudioWeeklyHours(parsed.data.hours)
    if (!result.data) {
      return errorResponse(result.error.message, 500, result.error.code)
    }

    revalidateTag("studio-hours", "max")

    return NextResponse.json({ data: result.data, error: null })
  } catch (err) {
    console.error("[api/admin/studio-hours PUT]", err)
    return errorResponse("Error interno del servidor", 500)
  }
}
