...
import { adminServiceCreateSchema } from "@/lib/validations/services"
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ data: { services: result.data }, error: null })
  } catch (err) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parseResult = adminServiceCreateSchema.safeParse(json)
    ...
    const result = await createService(parseResult.data)
    if (!result.data) {
      return errorResponse(result.error.message, 500, result.error.code)
    }
    ...
    return NextResponse.json(
      { data: { service: result.data }, error: null },
      { status: 201 }
    )
  } catch (err) {
