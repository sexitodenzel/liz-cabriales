...
import { isValidCategory, DEFAULT_BLOG_CATEGORY } from "@/lib/blog-categories"
...
export async function POST(request: Request) {
  try {
    ...
    const { title, slug, category, excerpt, cover_image, body: postBody, is_active, sort_order, products } =
    ...
    const cat = typeof category === "string" && isValidCategory(category) ? category : DEFAULT_BLOG_CATEGORY
    ...
    const result = await createBlogPost({
      ...
      body: typeof postBody === "string" ? postBody : undefined,
...
export async function PATCH(request: Request) {
  try {
    ...
    if (typeof fields.category === "string" && isValidCategory(fields.category)) update.category = fields.category
    ...
    if (typeof fields.body === "string") update.body = fields.body
    ...
    const result = await updateBlogPost(id, update)
    if (result.error) {
      return NextResponse.json({ data: null, error: { message: result.error } }, { status: 500 })
    }
