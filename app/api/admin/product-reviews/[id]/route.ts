...
export async function PATCH(
  ...
  try {
    ...
    if (typeof body.is_approved !== "boolean") {
      return errorResponse("is_approved debe ser booleano", 400, "VALIDATION_ERROR")
    }

    const result = await setProductReviewApproval(id, body.is_approved)
    if (result.error) {
    ...
    console.error("[api/admin/product-reviews PATCH] Error inesperado:", err)
...
export async function DELETE(
  ...
  try {
    ...
    console.error("[api/admin/product-reviews DELETE] Error inesperado:", err)
    return errorResponse("Error interno del servidor", 500)
  }
}
