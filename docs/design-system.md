...
**Una sola familia en todo el sitio (estilo OPI):** **Inter Tight** (grotesca de Google Fonts, réplica gratuita de Sharp Grotesk que usa opi.com), cargada en `app/layout.tsx` para cuerpo Y display. No agregar más ni reintroducir serifs. Los títulos usan la utility `font-display` (token `--font-display` en `globals.css`), que además aplica el tracking negativo `-0.02em` de la firma OPI. El nombre legacy `var(--font-playfair)` sigue vivo como **alias** que apunta a Inter Tight (`body { --font-playfair: var(--font-inter-tight) }` en `globals.css`), así que el código viejo no se tocó; en código nuevo preferir `font-display`.
...
- Chrome del sitio = **barra de anuncios + navbar**, todo dentro del mismo `<header id="site-navbar">` sticky (la barra se inyecta como slot desde `layout.tsx` → `SiteNavbarAuth` → `Navbar`). Navbar desktop: **una sola fila** estilo OPI — `lockup de marca (izquierda) · módulos (centro) · lupa → favoritos → cuenta → bolsa (derecha)`. El lockup es logo + wordmark "Liz Cabriales / STUDIO" (mismo par que el footer) sin color propio, para que el overlay del home lo pase a blanco por herencia
- Reparto horizontal: rejilla `grid-cols-[1fr_auto_1fr]`, NO flex. Las columnas laterales miden igual, así los módulos quedan centrados en la página aunque la marca y el grupo de iconos tengan anchos distintos (con flex quedaban pegados al logo)
- Módulos en **versalitas** (`uppercase`, ~13.5px, `tracking-[0.13em]`) y logo de 52px, siguiendo la referencia OPI
- Búsqueda: el navbar solo tiene la lupa. El campo vive en `MobileSearchOverlay` en las DOS variantes (`dropdown` = panel pegado al navbar en desktop ancho, cortina a pantalla completa en el resto) y el propio overlay lo enfoca al abrir. En desktop el campo se centra como bloque (tope 900px, márgenes idénticos) con la X fuera de flujo pegada a la derecha; el panel se ancla a `--site-chrome-bottom` (antes iba clavado a `top: 56` del navbar de dos filas) y lleva `scrollbar-gutter: stable both-edges` para que su propia barra de scroll no descentre el campo
- Indicador del módulo activo: `<nav>` va `self-stretch` para que la barrita se ancle a `bottom-0` = borde inferior del navbar (no pegada al texto); 2px, negra sobre marfil y blanca sobre la foto del home
- Alturas: `--navbar-h` = fila del navbar, **64px en todas las vistas** (sin excepción por breakpoint), `--announcement-h` = 36px si la barra existe, `--navbar-actual-h` = borde inferior del chrome expandido (64 sin barra, 100 con barra). Las tres se declaran con **literales en px** (nunca `calc()`): `getComputedStyle().getPropertyValue()` sustituye `var()` pero NO evalúa `calc()`, y `Navbar.tsx`/`ServiciosLanding.tsx` las parsean con `parseFloat`. El override va sobre `:root:has(#site-announcement-bar)` para que el valor se corrija solo cuando la barra llega por streaming o se apaga desde `/admin/announcements`
- Compresión por dirección de scroll (bajar comprime, subir muestra): lo único que se va es la barra de anuncios (`--navbar-collapse-shift` = `--announcement-h`); **el navbar entero es permanente en todas las vistas**. Navbar.tsx togglea `html.lc-nav-collapsed` y CSS transiciona el transform; clase `.navbar-follow-collapse` en los stickies que lo siguen, que quedan dockeados bajo el navbar y nunca salen de pantalla. Con overlay abierto (megamenú/carrito/búsqueda) el estado se CONGELA, no se fuerza el expandido — devolver el anuncio empujaría el megamenú 36px
- Guard del colapso: el sticky marcado `data-nav-collapse-guard` bloquea el colapso hasta estar pegado a su línea de dock — que es su propio `top` resuelto (computed style), así sirve tanto para barras pegadas al navbar (`top: var(--navbar-actual-h)`) como para el sidebar de curso que ancla 24px más abajo. Sin guard, el follow levanta al sticky fuera de sitio mientras aún va en el flujo (bug del primer scroll en `/academia/[id]`)
- Follow con fin de recorrido (sidebars de curso/producto): al agotar su columna el sticky se estaciona en el borde inferior y viaja con el contenido; el hook `useNavFollowParked` (`lib/hooks/use-nav-follow-parked.ts`) le pone `.lc-follow-parked` y el transform vuelve a 0 con la misma transición para que no descanse por encima del borde
